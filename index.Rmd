...
ggplot(mapping=aes(x = DATE,y=Data_housing,color=City,group=1, text = paste("Date:",DATE, "\n")))+
  ...
  labs(title="Housing Price Fluctuation of Three Major Cities In US",x="Time (1987-2021)",y="Case-Shiller index",caption="Sources:Federal Reserve Economic Data")
...
```{r}
raw_pop_BO <- read.csv("Boston-population-2021-06-22.csv")
raw_pop_NY <- read.csv("New York City-population-2021-06-22.csv")
raw_pop_SF <- read.csv("San Francisco-population-2021-06-22.csv")

pop_BO <- raw_pop_BO %>%
mutate(City="Boston") %>%
mutate(Data_pop= Population)
...
ggplot(mapping=aes(x = date,y=Data_pop,color=City,group=1, text = paste("Date:",date, "\n")))+
  geom_line()+
  scale_x_discrete(breaks=12)+
  labs(title="Population of Three Major Cities in US Overtime",x="Time (1987-2020)",y="Population",caption="Sources:Macrotrends.net")
